...
FilteredPaymentDebits1 = AllBalanceData %>%
  select(X.ID.1, X.CATEGORY, X.AMOUNT,X.TRANSACTION_DESC ) %>%
  filter( X.TRANSACTION_DESC =="Invalid Bank Account # Charge"| X.TRANSACTION_DESC =="Returned Check Serv. Charge 5%"| X.TRANSACTION_DESC =="Dishonored Wck Incorrect Acct"
         | X.TRANSACTION_DESC =="Dishonored Web Check 5%"| X.TRANSACTION_DESC =="Returned Check 5% Serv. Charge"
         | X.TRANSACTION_DESC =="Short Term Loan Advance Processing "| X.TRANSACTION_DESC =="Financial Aid Refund"
         | X.TRANSACTION_DESC =="Refund Distribution"| X.TRANSACTION_DESC =="Short Term Advance")
